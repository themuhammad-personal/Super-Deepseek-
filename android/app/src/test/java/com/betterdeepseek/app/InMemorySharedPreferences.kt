...
internal class InMemorySharedPreferences : SharedPreferences {
    ...
    /** `operator` so tests can use `prefs["key"] = "value"` like a real store. */
    operator fun set(key: String, value: String) {
        map[key] = value
    }

    /** `operator` so tests can read `prefs["key"]`. */
    operator fun get(key: String): String? = map[key]

    /** Mirror of the `contains` override below, for test readability. */
