...
class McpClientTest {
    ...
    fun `listTools retries after 404 by re-initializing`() {
        ...
        // Recorded requests come back in arrival order, so the retried tools/list
        // is the sixth (and last) one.
        val firstInit = server.takeRequest()
        assertEquals("initialize", JSONObject(firstInit.body.readUtf8()).getString("method"))
        server.takeRequest() // notifications/initialized
        val expired = server.takeRequest()
        assertEquals("tools/list", JSONObject(expired.body.readUtf8()).getString("method"))
        val secondInit = server.takeRequest()
        assertEquals("initialize", JSONObject(secondInit.body.readUtf8()).getString("method"))
        server.takeRequest() // notifications/initialized
        val retryReq = server.takeRequest()
        assertEquals("tools/list", JSONObject(retryReq.body.readUtf8()).getString("method"))
    ...
    fun `callTool sends name and arguments in tools call params`() {
        ...
        // init, notifications/initialized, then tools/call — take them in order.
        server.takeRequest()
        server.takeRequest()
