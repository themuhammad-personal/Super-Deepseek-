 ...
 *
 * Capabilities:
 * - SharedPreferences-backed key/value storage (chrome.storage.local)
 * - `fetch(payloadJson)` — every former background-service-worker message type:
 *   web/GitHub fetching, YouTube transcripts, locale/remote-config sync, MCP
 *   tools (list/call) and the DeepSeek Harness bridge (see [fetch])
 * - Asset URL resolution against the WebViewAssetLoader authority (chrome.runtime.getURL)
 * - Blob downloads to MediaStore/Downloads with viewer handoff
...
internal class WebViewBridge(
    ...
    /**
     * Native equivalents of the former background-service-worker message
     ...
     *
     * Supported types (1:1 with the former extension background service worker):
     * - bds-fetch-url -> { ok, status, html }
     * - bds-fetch-github-zip -> { ok, base64, status?, authRejected? }
     * - bds-fetch-github-commits -> { ok, commits, status?, authRejected?, rateLimited? }
     * - bds-get-youtube-transcript -> { ok, transcript } | { ok: false, error }
     * - BDS_UPDATE_LANGUAGES -> { success, writtenKeys, error? }
     * - BDS_WAIT_FOR_STARTUP -> { success, remoteStatus, remoteConfig }
     * - BDS_RESET_LANGUAGES -> { success }
     * - bds-mcp-list-tools -> { ok, tools } | { ok: false, error }
     * - bds-mcp-call -> { ok, result } | { ok: false, error }
     * - bds-api-proxy -> { ok, data, latency, streamed } | { ok: false, error, status }
     * - PING_HARNESS -> { ok, available, mode, pluginInfo?/error? }
     * - EXECUTE_HARNESS_TASK -> { ok, sessionId?, matchedPath?, error?, debug? }
     *
        ...
        try {
            ...
            when (val type = payload.optString("type")) {
                ...
                "BDS_UPDATE_LANGUAGES" -> {
                    val update = remoteDataSync.updateLanguages()
                    response.put("success", update.optBoolean("success"))
                    response.put("writtenKeys", update.optJSONArray("writtenKeys") ?: JSONArray())
                    update.opt("error")?.let { response.put("error", it) }
                }
                "BDS_WAIT_FOR_STARTUP" -> {
                    val startup = remoteDataSync.waitForStartup()
                    response.put("success", startup.optBoolean("success"))
                    response.put("remoteStatus", startup.optJSONObject("remoteStatus") ?: JSONObject())
                    response.put("remoteConfig", startup.optJSONObject("remoteConfig") ?: JSONObject())
                }
                "BDS_RESET_LANGUAGES" -> {
                    val reset = remoteDataSync.resetLanguages()
                    response.put("success", reset.optBoolean("success"))
                    reset.opt("error")?.let { response.put("error", it) }
                }
                "bds-mcp-list-tools" -> handleMcpListTools(payload, response)
                "bds-mcp-call" -> handleMcpCall(payload, response)
                "bds-api-proxy" -> handleApiProxy(payload, response)
                "PING_HARNESS" -> response.putAll(harnessClient.ping(payload.optString("baseUrl")))
                "EXECUTE_HARNESS_TASK" -> {
                    val payloadObj = payload.optJSONObject("payload") ?: JSONObject()
                    val taskResult = harnessClient.executeTask(payloadObj)
                    response.putAll(taskResult)
                }
                else -> {
    ...
    /** Copy every entry of [other] into the receiver (org.json has no portable putAll). */
    private fun JSONObject.putAll(other: JSONObject) {
        for (key in other.keys()) put(key, other.get(key))
    }

    /**
     * `bds-get-youtube-transcript` — port of the background SW handler that used
     * the `youtube-transcript` npm package. The JS side sends the extracted
     * 11-char `videoId` (URLs are tolerated as well).
     */
    private fun handleYouTubeTranscript(payload: JSONObject, response: JSONObject) {
        val videoIdOrUrl = payload.optString("videoId")
        if (videoIdOrUrl.isEmpty()) {
            response.put("ok", false)
            response.put("error", "No videoId provided.")
            return
        }
        try {
            val transcript = youtubeTranscript.fetchTranscript(videoIdOrUrl)
            response.put("ok", true)
            response.put("transcript", transcript)
        } catch (e: YouTubeTranscript.TranscriptException) {
            response.put("ok", false)
            response.put("error", e.message ?: "Failed to fetch transcript.")
        }
    }

    /**
     * `bds-mcp-list-tools` — native MCP `tools/list`. The `tools` field carries
     * the JSON-RPC result object (typically `{ "tools": [...] }`), exactly as
     * the background SW responded.
     */
    private fun handleMcpListTools(payload: JSONObject, response: JSONObject) {
        val serverUrl = payload.optString("serverUrl")
        val apiKey = payload.optString("apiKey")
        if (serverUrl.isEmpty()) {
            response.put("ok", false)
            response.put("error", "No serverUrl provided.")
            return
        }
        try {
            val tools = mcpClient.listTools(serverUrl, apiKey, BuildConfig.VERSION_NAME)
            response.put("ok", true)
            response.put("tools", tools)
        } catch (e: McpClient.McpException) {
            response.put("ok", false)
            response.put("error", e.message ?: "MCP tools/list failed.")
        } catch (e: Exception) {
            response.put("ok", false)
            response.put("error", e.message ?: "MCP tools/list failed.")
        }
    ...
    /**
     * `bds-api-proxy` — the API Playground's direct DeepSeek API calls
     * (chat/completions, completions, models, user/balance, with SSE streaming).
     * Port of `src/background/api-proxy.js`; the response object is passed
     * through unchanged (`{ ok, data, latency, streamed }` /
     * `{ ok: false, error, status }`).
     */
    private fun handleApiProxy(payload: JSONObject, response: JSONObject) {
        val request = payload.optJSONObject("request") ?: JSONObject()
        response.putAll(apiProxy.proxy(request))
    }

    /**
     * `bds-mcp-call` — native MCP `tools/call`. The `result` field carries the
     * JSON-RPC result object, exactly as the background SW responded.
     */
    private fun handleMcpCall(payload: JSONObject, response: JSONObject) {
        val serverUrl = payload.optString("serverUrl")
        val toolName = payload.optString("toolName")
        val apiKey = payload.optString("apiKey")
        if (serverUrl.isEmpty() || toolName.isEmpty()) {
            response.put("ok", false)
            response.put("error", "serverUrl and toolName are required.")
            return
        }
        val args = payload.optJSONObject("args") ?: JSONObject()
        try {
            val result = mcpClient.callTool(serverUrl, toolName, args, apiKey, BuildConfig.VERSION_NAME)
            response.put("ok", true)
            response.put("result", result)
        } catch (e: McpClient.McpException) {
            response.put("ok", false)
            response.put("error", e.message ?: "MCP tools/call failed.")
        } catch (e: Exception) {
            response.put("ok", false)
            response.put("error", e.message ?: "MCP tools/call failed.")
        }
