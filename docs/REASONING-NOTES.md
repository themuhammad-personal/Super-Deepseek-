...
> **Corrected 2026-09-25 (§7.4):** the claim that `api-proxy.js` was dead code
> was **wrong**. `src/background/index.js` opened with `import "./api-proxy.js";`
> — a side-effect import whose listener registered the `bds-api-proxy` (and
> `bds-api-proxy-abort`) message types. The API Playground
> (`src/content/api-playground/api-store.svelte.js`) sends exactly that message,
> so deleting the background folder broke every Playground request with
> "Unsupported bridge message type". The logic now lives in
> `android/.../ApiProxyClient.kt` (see §7.4).
...
> **Corrected 2026-09-25 (§7.3):** the runtime URLs were **not** untouched —
> they were rewritten from `…/main/extension/<file>` to `…/main/remote/<file>`,
> and the published upstream repository has no `remote/` directory, so every
> fetch returned 404 (pricing tier-2, announcement banner, remote config, and
> the native locale/remote-data sync). The URLs now point at the published
> `extension/…` paths again; the `remote/` copies are a repository-local mirror
> of the same payloads.
...
`src/background/api-proxy.js` (`proxyApiRequest`): **struck through — see the
correction below.** The original claim (no message type dispatched to it, no
importer, dead code, "the Playground calls the API directly from the page
context") was **false on every point**.

> **Corrected 2026-09-25 (§7.4):** `src/background/index.js` side-effect
> imported the module (`import "./api-proxy.js";`), which registered the
> `bds-api-proxy` listener; the Playground never called the API directly — it
> always went through `chrome.runtime.sendMessage({ type: "bds-api-proxy" })`.
> The ported implementation is `api-proxy` → `ApiProxyClient.kt`, dispatched
> from `WebViewBridge.fetch`.
...
Fix: `gradle-wrapper.properties` is committed (pinning the Gradle
distribution) and `gradle/wrapper/gradle-wrapper.jar` is fetched on demand by
`scripts/ensure-gradle-wrapper.js`, which both workflows run before
`./gradlew`. The bootstrap `curl` steps were replaced by that script, so the
wrapper location and version live in exactly one place.

> **Corrected 2026-09-25 (§7.1):** the wrapper JAR is **not** committed —
> `android/.gitignore` still ignores `gradle/wrapper/*` except the properties
> file (that is intentional: a ~40 kB binary that the script fetches). More
> importantly, the committed properties file *and* the script pinned
> `gradle-8.7.0-bin.zip`, which does not exist (the git tag is `v8.7.0`, the
> distribution asset is `gradle-8.7-bin.zip`), so `./gradlew` failed with a 404
> on every run — including CI, where the Kotlin test step failed and the APK
> step was skipped. Both are now pinned to `8.7`.
...
> **Corrected 2026-09-25 (§7.2, §7.5):** the build-id mechanism was *not*
> working. The workflow passed `-PbdsBuildId=<run>`, but Gradle property names
> are case-sensitive and the script read `findProperty("BdsBuildId")`, so
> `BuildConfig.BUILD_ID` was always `0` and the beta channel silently fell back
> to the timestamp heuristic. The `<!-- bds-build-id: … -->` marker that the
> update checker parses had also been dropped from the release body. Both are
> fixed, and the signing config now sits directly under `android {}` (it
> previously sat inside `buildTypes {}`, where it only compiled thanks to
> Kotlin's outer-receiver resolution).
