...
- `Gradle` runs the Kotlin unit tests and APK assembly for the Android shell.
  `scripts/ensure-gradle-wrapper.js` fetches the wrapper JAR on demand; the
  pinned distribution lives in `android/gradle/wrapper/gradle-wrapper.properties`
  (Gradle `8.7` — the distribution asset name has two components, while the git
  tag is `v8.7.0`).
...
- Cross-language contract tests live under `tests/integration/platform/`:
  `android-manifest.test.js` checks the manifest invariants and
  `android-bridge-contract.test.js` statically verifies that every
  `chrome.runtime.sendMessage({ type })` the JS side sends has a branch in
  `WebViewBridge.fetch`, and that every `window.AndroidBridge.method()` the JS
  side calls is a `@JavascriptInterface` method in `WebViewBridge.kt`.
